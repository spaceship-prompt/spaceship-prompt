...
你有能力制定和禁用 Spaceship 的模块。所有的选项必须在 `.zshrc` 文件中 Spaceship 主题后面来重写。

模块的配色可以在 [basic colors](https://wiki.archlinux.org/index.php/zsh#Colors) 和 [color codes](https://upload.wikimedia.org/wikipedia/commons/1/15/Xterm_256color_chart.svg) 中找。

**注意:** 文档中的符号 `·` 代表着空格 ` `, 它被用来清晰的表示什么时候默认选项以空格为开头或者结尾。

### 顺序

你能用 `SPACESHIP_PROMPT_ORDER` 选项来控制模块显示的顺序。 用 Zsh 数组语法定义你自己的提示符模块顺序。

模块顺序数组也定义了哪些模块需要被加载。如果你发现提示符很慢。你需要把你不用的模块关掉让它们不必要加载。
...
我们也能通过 `SPACESHIP_RPROMPT_ORDER` 控制右侧命令符的显示。 默认 `SPACESHIP_RPROMPT_ORDER` 是空的。

### Prompt

下面这组控制选项定义了命令符的行为和模块显示的标准参数。
...
该模块默认是关闭的。 如果你需要显示时间戳，将 `SPACESHIP_TIME_SHOW` 为 `true` 设置到你的 `.zshrc` 里。
...
默认，用户名仅当不等于 `$LOGNAME` 的时候才显示, 当你通过 SSH 链接或者你是 root 用户时。 Root 用户用 `SPACESHIP_USER_COLOR_ROOT` 颜色来高亮 (默认是红色)。
...
`SPACESHIP_USER_SHOW` 定义什么时候用户名模块被显示。 下面是可能的值:

| `SPACESHIP_USER_SHOW` | 本地显示 | 远程显示 |
| :-------------------: | :------------ | :------------- |
|        `false`        | Never         | Never          |
...
主机名仅当你建立SSH链接时显示。除非你用下面的选项改变配置。
...
目录模块总是被显示，并且会被截短成 `SPACESHIP_DIR_TRUNC` 的值。当你在一个仓库中它仅显示根目录以及在它里面的文件夹。
如果当前的目录是受保护的或者你没有写权限，默认会显示一个小锁。
...
Git 模块由 `git_branch` 和 `git_status` 子模块组成。它仅仅在仓库目录内显示。
...
Mercurial 模块由 `hg_branch` 和 `hg_status` 子模块组成。它仅仅在仓库目录内显示。
...
当可能时显示 Mercurial 书签, 否则显示 Mercurial 分支信息。
...
Mercurial 状态表示符仅在仓库有未提交更改的情况下显示。
...
> 当前仅仅对 [npm](https://www.npmjs.com/) 和 [cargo](https://crates.io/) 有效。 欢迎帮助我们提高该模块。

当该仓库是一个包时，包版本显示。

- **npm** — `npm` 包中包含 `package.json` 文件。 我们用 `jq`, `python` 来解析包版本，这样有更好的性能，同时用 `node` 作为补充。 安装 [jq](https://stedolan.github.io/jq/) 来 **提高性能** ([为什么?](./troubleshooting.zh.md#why-is-my-prompt-slow))
- **cargo** — `cargo` 包中包含 `Cargo.toml` 文件。 目前, 我们用 `cargo pkgid`, 它依赖 `Cargo.lock`. 因此如果包信息不显示，你可能需要执行 `cargo build` 用来生成 `Cargo.lock` 文件。

> **注意:** 这里的显示的是包的版本，而不是包管理器的版本。
...
Node.js 包仅仅显示在包含 `package.json` 文件, 或者 `node_modules` 文件夹, 或者 `.js` 后缀文件的目录中。

如果你设置了 `SPACESHIP_NODE_DEFAULT_VERSION`。当当前的版本和它一样时 Node.js 模块被隐藏。
...
Ruby 模块显示在包含 `Gemfile`, 或者 `Rakefile`,  `.rb` 后缀文件的目录中。
...
Elm 模块显示在包含 `elm.json`, `elm-package.json`, `elm-stuff` 目录, 或者 `.elm` 后缀文件的中。
...
Elixir 包仅仅显示在包含 `mix.exs` 文件, `.ex` or `.exs` 后缀文件的目录中。如果你设置了 `SPACESHIP_ELIXIR_DEFAULT_VERSION`。当当前的版本和它一样时 Elixir 模块被隐藏。
...
| `SPACESHIP_BATTERY_SHOW` | 阈值以下 | 阈值以上| 充电已满 |
| :----------------------: | :-------------- | :-------------- | :------------ |
...
如果这些控制还不够满足你的需求请阅读 Spaceship 的  [API 文档](./api.zh.md)。
